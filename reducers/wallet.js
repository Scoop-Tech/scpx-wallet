...
const handlers = {
    [WCORE_SET_ASSETS]: (state, action) => {
        utilsWallet.logMajor('orange','black', ` WCORE_SET_ASSETS, len= `, action.payload.assets.length)
        return { assets: action.payload.assets, owner: action.payload.owner }
    },

    [WCORE_SET_ASSETS_RAW]: (state, action) => {
        utilsWallet.logMajor('orange','black', ` WCORE_SET_ASSETS_RAW, len= `, action.payload.length)
    ...
    [WCORE_SET_ENRICHED_TXS_MULTI]: (state, action) => {
        ...
        utilsWallet.logMajor('orange','black', ` WCORE_SET_ENRICHED_TXS_MULTI ${symbol} x${addrTxs.length} `)
    ...
    [WCORE_SET_ADDRESSES_FULL_MULTI]: (state, action) => {
        ...
        utilsWallet.logMajor('orange','black', ` WCORE_SET_ADDRESSES_FULL_MULTI ${action.payload.symbol} x${action.payload.newAddresses.length} `)
    ...
    [WCORE_PUSH_LOCAL_TX]: (state, action) => {
        utilsWallet.logMajor('orange','black', ` LOCAL_TX - PUSH - ${action.payload.symbol}, txid=${action.payload.tx.txid} `)
        var assets = _.cloneDeep(state.assets)
        ...
        utilsWallet.logMajor('orange','black', `LOCAL_TX - PUSH DONE - ${action.payload.symbol} asset.local_txs=`, asset.local_txs)
        return { ...state, assets }
    },
}
